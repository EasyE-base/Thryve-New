    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert data migration specialist for fitness studio management systems. Parse and structure fitness studio data into standardized formats. ALWAYS respond with valid JSON only, no additional text."
          },
          {
            role: "user",
            content: prompt
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          throw new Error(`AI response contains no JSON: ${content.substring(0, 100)}...`)
        }
      }
