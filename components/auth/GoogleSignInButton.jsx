...
async function postLoginSideEffects() {
  try {
...
export default function GoogleSignInButton({ next = '/' }) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await postLoginSideEffects()
      window.location.assign(next)
    } catch (e) {
      const code = e?.code || ''
      if (code === 'auth/unauthorized-domain' || code === 'auth/popup-blocked' || code === 'auth/popup-closed-by-user') {
  ...
  return (
    <button type="button" onClick={handleGoogle} className="w-full rounded-md border px-4 py-2 font-medium hover:bg-gray-50" disabled={loading}>
